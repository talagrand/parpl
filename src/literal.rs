...
#[inline]
fn validate_unicode_codepoint(code_point: u32) -> Result<char, Error> {
...
fn process_hex_escape(chars: &mut std::str::Chars) -> Result<u32, Error> {
...
fn process_unicode_short_escape(chars: &mut std::str::Chars) -> Result<char, Error> {
...
fn process_unicode_long_escape(chars: &mut std::str::Chars) -> Result<char, Error> {
...
fn process_octal_escape(chars: &mut std::str::Chars, first_digit: char) -> Result<u8, Error> {
...
fn collect_hex_digits(chars: &mut std::str::Chars, count: usize) -> Result<String, Error> {
...
pub fn parse_uint(s: &str) -> Result<u64, Error> {
    // Parse based on prefix
...
pub fn process_bytes_escapes<'arena>(
    s: &str,
    interner: &mut StringInterner<'arena>,
) -> Result<&'arena [u8], Error> {
    // Fast path: no escapes - just convert UTF-8 string to bytes
    if !s.contains('\\') {
        return Ok(interner.alloc_bytes(s.as_bytes()));
    }
    ...
    Ok(interner.alloc_bytes(&result))
}
    ...
    match raw {
        ...
        RawLiteral::Bytes(content, is_raw, _quote_style) => {
            if *is_raw {
                ...
                Ok(Literal::Bytes(interner.alloc_bytes(content.as_bytes())))
            } else {
                // Process escape sequences
                let processed = process_bytes_escapes(content, interner)?;
...
mod tests {
    ...
    fn test_string_escapes_simple() {
        ...
        assert_eq!(
            process_string_escapes("hello\\nworld", &mut interner).unwrap(),
            "hello\nworld"
        );
        assert_eq!(
            process_string_escapes("tab\\there", &mut interner).unwrap(),
            "tab\there"
        );
        assert_eq!(
            process_string_escapes("quote\\\"test", &mut interner).unwrap(),
            "quote\"test"
        );
    }
    ...
    fn test_string_escapes_hex() {
        ...
        assert_eq!(process_string_escapes("\\x41", &mut interner).unwrap(), "A");
        assert_eq!(
            process_string_escapes("\\xFF", &mut interner).unwrap(),
            "\u{FF}"
        );
    }
    ...
    fn test_string_escapes_unicode() {
        ...
        assert_eq!(
            process_string_escapes("\\u0041", &mut interner).unwrap(),
            "A"
        );
        assert_eq!(
            process_string_escapes("\\U00000041", &mut interner).unwrap(),
            "A"
        );
        assert_eq!(
            process_string_escapes("\\u2764", &mut interner).unwrap(),
            "❤"
        );
    }
    ...
    fn test_string_escapes_octal() {
        ...
        assert_eq!(process_string_escapes("\\101", &mut interner).unwrap(), "A");
        assert_eq!(
            process_string_escapes("\\377", &mut interner).unwrap(),
            "\u{FF}"
        );
    }
    ...
    fn test_string_escapes_invalid_surrogate() {
        ...
        assert!(process_string_escapes("\\uD800", &mut interner).is_err());
        assert!(process_string_escapes("\\uDFFF", &mut interner).is_err());
    }
    ...
    fn test_string_escapes_invalid_sequence() {
        ...
        assert!(process_string_escapes("\\s", &mut interner).is_err());
        assert!(process_string_escapes("\\", &mut interner).is_err());
    }
    ...
    fn test_no_escapes_fast_path() {
        ...
        let result = process_string_escapes("hello world", &mut interner).unwrap();
    ...
    fn test_process_literal_integers() {
        ...
        assert_eq!(
            process_literal(&raw, &mut interner).unwrap(),
            Literal::Int(123)
        );
    ...
    fn test_process_literal_floats() {
        ...
        if let Literal::Float(val) = process_literal(&raw, &mut interner).unwrap() {
    ...
    fn test_process_literal_bools_and_null() {
        ...
        assert_eq!(process_literal(&raw, &mut interner).unwrap(), Literal::Null);
    }
    ...
    fn test_bytes_invalid_utf8() {
        ...
        let result = process_literal(&raw, &mut interner).unwrap();
