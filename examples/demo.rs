// Comprehensive Cello Demo
//
// This example demonstrates the key features of Cello:
...
fn main() -> Result<()> {
    ...
    CelloBuilder::new().parse_scoped("1 + 2 * 3", |ctx| {
        let ast = ctx.ast()?;
    ...
    CelloBuilder::new().parse_scoped("true && false", |ctx| {
        let ast = ctx.ast()?;
    ...
    // Syntax error
    match CelloBuilder::new().parse_scoped("1 + + 2", |ctx| ctx.ast().map(|_| ())) {
        Ok(_) => println!("   ✗ Should have failed"),
        Err(e) => println!("   ✓ Syntax error caught: {}", e),
    }

    // Reserved word error
    match CelloBuilder::new().parse_scoped("for", |ctx| ctx.ast().map(|_| ())) {
        Ok(_) => println!("   ✗ Should have failed"),
    ...
    for expr in examples {
        match CelloBuilder::new().parse_scoped(expr, |ctx| ctx.ast().map(|_| ())) {
